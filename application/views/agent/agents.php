<script type="text/javascript">
	...
	$.ajax({
		type: 'POST',
		url: '<?php echo base_url(); ?>Agent/ajaxagent/'+page_num,
...
<h1>Agents</h1>
<a href="<?= base_url()?>Agent/Add_agent">Add</a>
<input type="text" id="agents_keyword"><button type="button" id="search_agents">Search</button>
<button type="button" id="export_agents">Export Data</button>
...
<div id="postList">
  ...
  <div class="modal fade" id="myModal" role="dialog">
    <!-- <div class="modal-dialog"> -->
     ...
     <form action="<?= base_url()?>Agent/UpdateAgent" method="post" id="edit_agent_form">
     	<input type="hidden" name="id" id="edit_id">
	<select name="agent_group" id="agent_group">
		<?php
			foreach ($group_data as $group_data_value) { ?>
				<option value="<?= $group_data_value['id'];?>"><?= $group_data_value['name'];?></option>	
		<?php	}
		?>
	</select>									
	...
	<select name="port" id="port">
		<?php
			foreach ($port_data as $port_data_value) { ?>
				<option value="<?= $port_data_value['id'];?>"><?= $port_data_value['name'];?></option>	
		<?php	}
		?>
	</select>
